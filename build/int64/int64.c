...
toint64(lua_State *L, int index) {
	...
	case LUA_TUSERDATA: {
		PbcInteger64* p = lua_touserdata(L, index);
		n = p->data.i64;
		break;
	}
	default:
...
int64_add(lua_State *L) {
	int64_t a = toint64(L,1);
	int64_t b = toint64(L,2);
	_pushint64(L, a+b);
...
LUA_API int luaopen_int64(lua_State* L);
